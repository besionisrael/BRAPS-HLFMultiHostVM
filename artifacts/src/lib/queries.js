...
class QueryUtils {
    ...
    // =========================================================================================
    /**
    * Get Asset History for a vdx paper
    * @param {String} issuer the vdx issuer
    * @param {String} paperNumber vdx paper number
    */
    ...
    */
    ...
    // queryKeyByOperator queries for assets based on a passed in operator.
    // This is an example of a parameterized query accepting a single query parameter (operator).
    // Only available on state databases that support rich query (e.g. CouchDB)
    // =========================================================================================
    /**
    * queryKeyByOperator vdx paper
    * @param {String} operator vdx paper operator
    */
    async queryKeyByOperator(operator) {
        //  
        let self = this;
        if (arguments.length < 1) {
            throw new Error('Incorrect number of arguments. Expecting operator name.');
        }
        let queryString = {};
        queryString.selector = {};
        //  queryString.selector.docType = 'indexOperatorDoc';
        queryString.selector.operator = operator;
        // set to (eg)  '{selector:{operator:RQ}}'
    ...
    // If this is not desired, follow the queryKeyByOperator example for parameterized queries.
    // Only available on state databases that support rich query (e.g. CouchDB)
    // example passed using VS Code ext: ["{\"selector\": {\"operator\": \"RQ\"}}"]
    // =========================================================================================
    /**
    * query By AdHoc string (vdx paper)
                        ...
                        try {
                            ...
                            switch (jsonRes.Value.currentState) {
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    jsonRes.Value.currentState = 'CHECKED';
                                    break;
                                case 3:
                                    jsonRes.Value.currentState = 'TREATED';
                                    break;
                                case 4:
                                    jsonRes.Value.currentState = 'PAID';
                                    break;
                                case 5:
